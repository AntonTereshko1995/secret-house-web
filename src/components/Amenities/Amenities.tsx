function Amenities() {
    const amenities = [
        {
            icon: (
                <svg className="w-8 sm:w-10 h-8 sm:h-10 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
                    {amenities.map((amenity, index) => (
                        <div
                            key={index}
                            className="group text-center p-4 sm:p-5 bg-luxury-card transition-all duration-500 border border-yellow-600/20 hover:border-yellow-600/50 shadow-luxury hover:shadow-luxury-hover hover:scale-105"
                        >
                            <div className="mb-3 transform group-hover:scale-110 transition-transform duration-500 flex justify-center">
                                {amenity.icon}
                            </div>
                            <div className="h-px w-10 bg-gradient-to-r from-transparent via-yellow-600 to-transparent mx-auto mb-2"></div>
                            <h3 className="text-sm font-bold text-yellow-600 mb-1.5 uppercase tracking-wider">
                                {amenity.title}
                            </h3>
