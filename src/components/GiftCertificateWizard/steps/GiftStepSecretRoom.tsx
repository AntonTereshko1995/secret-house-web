...
function GiftStepSecretRoom({ giftData, updateGiftData, nextStep, prevStep }: GiftStepProps) {
  const [hasSecretRoom, setHasSecretRoom] = useState(giftData.hasSecretRoom)
  const price = giftData.secretRoomPrice

  const handleNext = () => {
    logger.info('gift_select', { step: 'secret-room', hasSecretRoom, price: hasSecretRoom ? price : 0 })
    updateGiftData({ hasSecretRoom })
    nextStep()
  }
  ...
  return (
    <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 shadow-2xl">
      ...
      <p className="text-gray-400 mb-3 text-sm">
        Добавить секретную комнату к сертификату? (необязательно)
      </p>
...
export default GiftStepSecretRoom
