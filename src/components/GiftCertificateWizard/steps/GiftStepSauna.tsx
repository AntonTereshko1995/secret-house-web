...
function GiftStepSauna({ giftData, updateGiftData, nextStep, prevStep }: GiftStepProps) {
  const [hasSauna, setHasSauna] = useState(giftData.hasSauna)
  const price = giftData.saunaPrice

  const handleNext = () => {
    logger.info('gift_select', { step: 'sauna', hasSauna, price: hasSauna ? price : 0 })
    updateGiftData({ hasSauna })
    nextStep()
  }
  ...
  return (
    <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 shadow-2xl">
      ...
      <p className="text-gray-400 mb-3 text-sm">
        Добавить сауну к сертификату? (необязательно)
      </p>
